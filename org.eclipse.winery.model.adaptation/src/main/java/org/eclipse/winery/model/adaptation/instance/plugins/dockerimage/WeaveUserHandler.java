...
public class WeaveUserHandler implements ImageRefinementHandler {
    private static final String IMAGE_ID_WEAVE_USER = "weaveworksdemos/user:0.4.7";
    private static final QName QNAME_WEAVE_USER_CONTAINER = QName.valueOf(
        "{https://examples.opentosca.org/edmm/nodetypes}Weaveworks-User-Container");
    ...
    public Set<String> getTargetImages() {
        return Stream.of(IMAGE_ID_WEAVE_USER).collect(Collectors.toSet());
    }

    @Override
    public Set<QName> getProhibitedTypes() {
        return Stream.of(QNAME_WEAVE_USER_CONTAINER).collect(Collectors.toSet());
    }
    ...
    public void handleNode(
        ...
        dockerContainer.setType(QNAME_WEAVE_USER_CONTAINER);
