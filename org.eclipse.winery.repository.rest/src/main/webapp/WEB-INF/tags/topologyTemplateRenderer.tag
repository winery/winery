<%--~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  ...
  ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~--%>
<%@tag
    description="Renders a toplogytemplate. This tag is used to render a topology template readonly. The topoology modeler does the rendering on itself."
    pageEncoding="UTF-8" %>
...
<%@attribute name="topology" required="true" description="the topology template to be rendered"
             type="org.eclipse.winery.model.tosca.TTopologyTemplate" %>
...
<%-- required for vShowError --%>
<link type="text/css" href="${w:topologyModelerURI()}/components/pnotify/jquery.pnotify.default.css" media="all"
      rel="stylesheet"/>
<link type="text/css" href="${w:topologyModelerURI()}/components/pnotify/jquery.pnotify.default.icons.css" media="all"
      rel="stylesheet"/>
...
<c:if test="${not empty fullscreen}">
    <link rel="stylesheet" href="${pageContext.request.contextPath}/css/topologyTemplateRendererFullscreen.css"/>
</c:if>
<c:if test="${not empty additonalCSS}">
    <link rel="stylesheet" href="${additonalCSS}"/>
</c:if>
...
<div id="editorArea">
    <div id="templateDrawingArea">
        ...
        %>
        <nt:nodeTemplateRenderer top="<%=top%>" left="<%=left%>" nodeTemplate="<%=nodeTemplate%>"
                                 repositoryURL="${repositoryURL}" uiURL="<%=uiURL%>" client="<%=client%>"
                                 relationshipTypes="<%=relationshipTypes%>"
                                 topologyModelerURI="${w:topologyModelerURI()}/"/>
        <%
            }
        ...
        <tmpl:registerConnectionTypesAndConnectNodeTemplates repositoryURL="${repositoryURL}"
                                                             relationshipTypes="<%=relationshipTypes%>"
                                                             relationshipTemplates="<%=relationshipTemplates%>"
                                                             ondone="onDoneRendering();" readOnly="true"/>
    </div>
</div>
