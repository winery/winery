...
public class NpmAnalyzer implements CommandAnalyzer {
    public List<Component> analyze(String command) {
        if (!(command.contains(" install ") || command.contains(" i ") || command.contains("add"))) {
            return new ArrayList<>();
        }
        command = command.replace(Commands.Npm.asString(), "");
        ...
        for (String word : words) {
            ...
            if (word.equals("install") || word.equals("i") || word.equals("add")) {
                packages.clear();
            }
        }
