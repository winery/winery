...
import { WineryUrlBasedLicense } from './wineryUrlBasedLicense';
import { Observable } from 'rxjs/Rx';
import { forkJoin } from 'rxjs/index';

export class WineryLicenceFactory {
    ...
    loadLicenseTexts(http: HttpClient): Observable<string[]> {
        ...
        const allObservables = this._allLicences.map(license => {
            ...
            return http.get(license.LICENSE_FILE_URL, { headers: headers, responseType: 'text' })
                .map((result: string) => {
    ...
    getLicenseByName(name: string): WineryLicense {
        return this._allLicences.find(item => item.LICENCE_NAME.toLowerCase() === name.toLowerCase());
    }
