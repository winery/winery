...
public class NodeTemplateResource extends TEntityTemplateResource<TNodeTemplate> implements INodeTemplateResourceOrNodeTypeImplementationResource {
	...
	/**
	 * 
	 * @param nodeTemplate the node template this resource is modeling
	 * @param list the list the node template is contained in. It is NOT of type
	 *            "List<TNodeTemplate>" as the list holding
	 * @param idx
	 * @param serviceTemplateResource
	 */
	public NodeTemplateResource(TNodeTemplate nodeTemplate, List<TNodeTemplate> list, int idx, ServiceTemplateResource serviceTemplateResource) {
		super(nodeTemplate, list, idx, serviceTemplateResource);
		this.nodeTemplate = nodeTemplate;
		assert (this.template == this.nodeTemplate);
	}
	
	@Path("deploymentartifacts/")
	public DeploymentArtifactsResource getDeploymentArtifacts() {
		return new DeploymentArtifactsResource(this.nodeTemplate, this);
	}
	
	@GET
	@RestDoc(methodDescription="* The following methods are currently *not* used by the topology modeler.<br />" +
	"The modeler is using the repository client to interact with the repository")
	@Path("minInstances")
	public String getMinInstances() {
		return Integer.toString(this.nodeTemplate.getMinInstances());
	}
	...
	public Response setMinInstances(@FormParam(value = "minInstances") String minInstances) {
		...
		this.nodeTemplate.setMinInstances(min);
	...
	public String getMaxInstances() {
		return this.nodeTemplate.getMaxInstances();
	}
	...
	public Response setMaxInstances(@FormParam(value = "maxInstances") String maxInstances) {
		...
		this.nodeTemplate.setMaxInstances(maxInstances);
		return BackendUtils.persist(this.res);
	}
	...
	public String getX() {
		Map<QName, String> otherAttributes = this.nodeTemplate.getOtherAttributes();
	...
	public Response setX(String x) {
		this.nodeTemplate.getOtherAttributes().put(this.qnameX, x);
		return BackendUtils.persist(this.res);
	}
	...
	public Response setY(String y) {
		this.nodeTemplate.getOtherAttributes().put(this.qnameY, y);
		return BackendUtils.persist(this.res);
	}
