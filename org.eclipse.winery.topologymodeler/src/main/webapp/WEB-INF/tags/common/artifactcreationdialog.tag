...
<%@attribute name="onSuccessfulArtifactCreationFunction" required="true"
             description="javascript code to be executed when the artifact has been successfully created. Parameter: artifactInfo" %>
<%@attribute name="allArtifactTypes" required="true" type="java.util.Collection"
             description="All available artifact types" %>
...
<%@attribute name="URL" required="true"
             description="the URL of the artifact collection. May also be a function returning the correct URL (used at the topology modeler). I.e., it is an expression being evaluated" %>

<%@attribute name="isDeploymentArtifact" required="true" type="java.lang.Boolean"
             description="Is this dialog used to create deployment artifacts?" %>
        ...
        $.ajax({
            ...
            success: function (data, textStatus, jqXHR) {
                ...
                /*if (autoCreateArtifactTemplate) {
                    ...
                    var artifactTemplateURL = makeArtifactTemplateURL("
                ${repositoryURL}", aritfactTemplateNS, artifactTemplateName);
                ...
                <form id="add${name}ArtifactForm" enctype="multipart/form-data">
                    <fieldset>
                        ...
                        <div class="form-group">
                            <label>Name</label>
                            <input class="form-control" name="artifactName" id="artifactName" type="text"
                        ...
                        <c:if test="${not isDeploymentArtifact}">
                            <div class="form-group">
                                ...
                                <select name="interfaceName" id="interfaceName" class="form-control"
                                        onchange="updateListContent(this.value, '#operationName', dependendSelects, interfaceOpData);">
                        ...
                        <div class="radio">
                            <label>
                                <input type="radio" name="artifactTemplateCreation" value="createArtifactTemplate"
                                       checked="checked" id="createArtifactTemplateInput">Create Artifact
                                Template</input>
                            </label>
                            <p class="help-block">Check if you want to upload <strong>new</strong> files, you do not
                                want to reuse existing files and you do not point to an image library.</p>
                        </div>
                        <div class="radio">
                            <label>
                                <input type="radio" name="artifactTemplateCreation" value="linkArtifactTemplate">Link
                                Artifact Template</input>
                            </label>
                            <p class="help-block">Check if you want to reuse existing files.</p>
                        </div>
                        <div class="radio">
                            <label>
                                <input type="radio" name="artifactTemplateCreation" value="skipArtifactTemplate">Do not
                                create an artifact template</input>
                            </label>
                    ...
                    <fieldset>
                        ...
                        <fup:artifacttemplateselection allNamespaces="${allNamespaces}" repositoryURL="${repositoryURL}"
                                                       defaultNSForArtifactTemplate="${defaultNSForArtifactTemplate}"/>
...
<div class="modal fade" id="addFilesToArtifactTemplate">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
                <h4 class="modal-title">Add files to artifact template <a id="artifactTemplateNameAtUploadFiles"></a>
                </h4>
            </div>
            <div class="modal-body">
