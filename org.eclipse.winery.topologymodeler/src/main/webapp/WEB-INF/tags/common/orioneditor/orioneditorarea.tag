...
<%@attribute name="initialtext" required="false"
             description="The value to put in the editor. Can be also passed as body of this tag" %>
...
<div>
    <div id="${areaid}" class="orionxmleditordiv" <c:if test="{$not empty hidden}">style="display: none;"</c:if>>
        <pre>${wc:escapeHtml4(initialtext)}<jsp:doBody/></pre>
    </div>
    <c:if test="${empty withoutsavebutton}">
        <button class="btn btn-primary" type="button" onclick="window.winery.orionareas['${areaid}'].save(this);"
