...
<%@attribute name="repositoryURL" required="true" type="java.lang.String"
             description="The URL of winery's repository" %>
<%@attribute name="uiURL" required="true" type="java.lang.String" description="The URL of winery's UI" %>
<%@attribute name="topologyModelerURI" required="false" type="java.lang.String"
             description="The URL of winery topology modeler's URI - required for images/. Has to end with '/'. Can be left blank." %>
<%@attribute name="relationshipTypes" description="the known relationship types" required="true"
...
<%@tag import="org.eclipse.winery.model.tosca.*" %>
...
<div class="NodeTemplateShape unselectable <%=nodeTypeCSSName%> <%if (paletteMode){%> hidden<%}%>"
     id="<%=visualElementId%>" style="left: <%=left%>px; top: <%=top%>px">
    <div class="headerContainer">
        <img class="icon"
             onerror="var that=this; require(['winery-common-topologyrendering'], function(wct){wct.imageError(that);});"
             src="<%=repositoryURL%>/nodetypes/<%=Util.DoubleURLencode(nodeTypeQName)%>/visualappearance/50x50"/>
        <%
            String name;
            if (paletteMode) {
    ...
    <div class="endpointContainer">
        ...
        %>
        <div
            class="connectorEndpoint <%=Util.makeCSSName(relationshipType.getTargetNamespace(), relationshipType.getName())%>">
            <div
                class="connectorBox <%=Util.makeCSSName(relationshipType.getTargetNamespace(), relationshipType.getName())%>_box"></div>
            <div class="connectorLabel"><%=relationshipType.getName()%>
            </div>
        </div>
        <%
            }
    ...
    <div class="targetLocationContainer">
        ...
        <div class="content">
            <a class="thetargetlocation" href="#" data-type="text"
               data-title="Enter the target location"><%=ModelUtilities.getTargetLabel(nodeTemplate).orElse("")%>
            </a>
        </div>
    </div>
    ...
    <div class="deploymentArtifactsContainer">
        ...
        <div class="content">
            ...
            %>
            <div class="deploymentArtifact row"
                 onclick="showDeploymentArtifactInformation('<%=visualElementId%>', '<%=deploymentArtifact.getName()%>');">
                <textarea
                    class="hidden"><%=org.eclipse.winery.common.Util.getXMLAsString(org.eclipse.winery.model.tosca.TDeploymentArtifact.class, deploymentArtifact)%></textarea>
                <div class="col-xs-4 overflowhidden deploymentArtifact name"><%=deploymentArtifact.getName()%>
                </div>
    ...
    %>
    <ntrq:reqs list="<%=reqList%>" repositoryURL="${repositoryURL}" pathToImages="${topologyModelerURI}images/"
               client="${client}"/>
    ...
    %>
    <ntrq:caps list="<%=capList%>" repositoryURL="${repositoryURL}" pathToImages="${topologyModelerURI}images/"
               client="${client}"/>
