...
<%@tag language="java" pageEncoding="UTF-8"
       description="This tag is used to render Topology Templates for selection in a dialog." %>
...
<div id="topologyTemplateSelector">
        ...
        <div id="proposaldrawingarea">
            <div id="<%=counter%>">
                ...
                <nt:nodeTemplateRenderer client="<%=client%>"
                                         ...
                                         nodeTemplate="<%=nodeTemplate%>" top="<%=Integer.toString(topCounter)%>"
                                         left='<%="0"%>'/>
    ...
    <br>
    <input name="<%=id%>" id="<%=id%>" type="checkbox" value='<%=stringWriter.toString()%>' onclick="onClick<%=id%>()">
    Save this Topology &nbsp; &nbsp;
    <script>
        /**
    ...
    <input disabled="disabled" name='<%=id + "overwrite"%>' id='<%=id + "overwrite"%>' type="checkbox"
           onclick='onClick<%=id + "overwrite"%>()'> Overwrite current Topology &nbsp; &nbsp;
    <script>
        /**
    ...
    <input disabled="disabled" name='<%=id + "newWindow"%>' id='<%=id + "newWindow"%>' type="checkbox"> Open in new
    Window <br> <br>
    <p>Name: <input disabled="disabled" id='<%=id + "name"%>' name='<%=id + "name"%>' value="<%=topologyName%>"
                    type="text" size="30" maxlength="30"></p>
    <p>Namespace: <input disabled="disabled" id='<%=id + "namespace"%>' value="<%=topologyNamespace%>"
                         name='<%=id + "namespace"%>' type="text" size="50" maxlength="60"></p>
    <%
            counter++;
            i++;
        }
                ...
                if (document.getElementById(IDs[i]).checked) {
                    ...
                    if (validURIregexp.test(namespace) || overwrite) {
                        ...
                        $.post("jsp/topologyCompletion/topologySaver.jsp", {
                                topology: document.getElementById(IDs[i]).value,
                                templateURL: "<%=templateURL%>",
                                repositoryURL: "<%=repositoryURL%>",
                                topologyName: name,
                                topologyNamespace: namespace,
                                overwriteTopology: overwrite
                            },
                            function (data) {
                                if (openInNewWindow) {
