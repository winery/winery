...
<%@tag language="java" pageEncoding="UTF-8"
       description="This tag is used to render Node and Relationship Templates for selection in a dialog." %>
...
<%@attribute name="choices"
             type="java.util.Map<org.eclipse.winery.model.tosca.TNodeTemplate, java.util.Map<org.eclipse.winery.model.tosca.TNodeTemplate, java.util.List<org.eclipse.winery.model.tosca.TEntityTemplate>>>" %>
...
<div id="nodeTemplateSelector">
    ...
    <div id="proposalEditorArea">
        <div id="proposaldrawingarea">
            <%
                ...
                topCounter = 0;
            %>
            <nt:nodeTemplateRenderer client="<%=client%>"
                                     relationshipTypes="<%=client.getAllTypes(TRelationshipType.class)%>"
                                     repositoryURL='<%=repositoryURL%>' nodeTemplate="<%=nt%>"
                                     top="<%=Integer.toString(topCounter)%>" left='<%="0"%>'/>
            <script>
            ...
            <nt:nodeTemplateRenderer client="<%=client%>"
                                     relationshipTypes="<%=client.getAllTypes(TRelationshipType.class)%>"
                                     repositoryURL='<%=repositoryURL%>' nodeTemplate="<%=choice%>"
                                     top="<%=Integer.toString(topCounter)%>" left='<%="0"%>'/>
            <script>
                //Map IDs here
                <%
    ...
    <p> There are several possible Relationship Templates to connect the Node Templates <%=nt.getName()%>
        and <%=choice.getName()%>. Please choose at least one connection: </p>
    <%
        }
        for (TEntityTemplate rtChoice : entityTemplates.get(choice)) {
            ...
            if (entityTemplates.get(choice).size() > 1) {
    %>
    <input checked="checked" id="<%=connector.getName()%>" name="<%=connector.getName()%>" type="checkbox"
           value="<%=connector.getName()%>"> <%=connector.getName()%> <br/>
    <%
        }
    ...
    %>
    <br>
    <button type="button" class="btn btn-primary btn-default" data-dismiss="modal" id="<%=id%>"
            value='<%=choice.getName()%>' onclick="onSelected<%=choice.getName()%>()">Use
        Template: <%=choice.getName()%>
    </button>
    <script>
        ...
        function onSelected<%=choice.getName()%>() {
            ...
            if (SelectedItems.length == 0) {
                ...
                $.post("jsp/topologyCompletion/selectionHandler.jsp", {
                        topology: topology,
                        allChoices: choices,
                        selectedNodeTemplates: selectedNodeTemplates,
                        selectedRelationshipTemplates: selectedRelationshipTemplates
                    },
                    function (data) {
    ...
    <br>
    <br>
    <button type="button" class="btn btn-primary btn-default" data-dismiss="modal" id="cancel"
            onclick="onSelectedCancel()">Cancel Automatic Completion
    </button>
    ...
    <script>
        ...
        function onSelectedCancel() {
            $.post("jsp/topologyCompletion/topologySaver.jsp", {
                    topology: topology,
