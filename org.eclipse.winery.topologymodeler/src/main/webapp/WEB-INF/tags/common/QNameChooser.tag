...
<%@attribute name="allQNames" required="true" type="java.util.Collection"
             description="Collection&lt;QName&gt; of all available QNames" %>
<%@attribute name="includeNONE" required="false" type="java.lang.Boolean"
             description="Should (none) be included as option?" %>
...
<div class="form-group">
    <c:if test="${not empty labelOfSelectField}"><label for="${idOfSelectField}"
                                                        class="control-label">${labelOfSelectField}:</label></c:if>
    <select id="${idOfSelectField}" name="${idOfSelectField}" class="form-control">
        <c:if test="${includeNONE}">
