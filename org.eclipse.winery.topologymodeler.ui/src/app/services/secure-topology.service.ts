...
@Injectable()
export class SecureTopologyService {
     ...
     */
    secureTopologyTemplate(firstComponent: string, secondComponent: string, backendService: BackendService,
                           errorHandler: ErrorHandlerService): void {
        this.alert.info('', 'Securing topology...');
        backendService.secureTopology(firstComponent, secondComponent).subscribe(res => {
                if (res.ok) {
                    const url = res.headers.get('location');
                    this.alert.success('', 'Successfully secured topology.');
